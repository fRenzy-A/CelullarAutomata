        ...
        if (squareGrid != null)
        {
            for (int i = 0; i <squareGrid.squares.GetLength(0); i++)
            {
                for (int j = 0; j < squareGrid.squares.GetLength(1); j++)
                {
                    ...
                    Gizmos.DrawCube(squareGrid.squares[i, j].centreTop.position, Vector3.one * .4f);
                    Gizmos.DrawCube(squareGrid.squares[i, j].centreRight.position, Vector3.one * .4f);
                    Gizmos.DrawCube(squareGrid.squares[i, j].centreBottom.position, Vector3.one * .4f);
                    Gizmos.DrawCube(squareGrid.squares[i, j].centreLeft.position, Vector3.one * .4f);
                }
            }
        }
    }
